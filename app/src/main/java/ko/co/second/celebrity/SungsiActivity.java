...
public class SungsiActivity extends AppCompatActivity {
    ...
    private List<Store> sungsiStores; // 유튜버 성시경의 스토어 리스트
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sungsiStores = storeManager.getStoresByYoutubeName("성시경");

        // storeName만 추출하여 리스트에 담기
        storeNames = new ArrayList<>();
        for (Store store : sungsiStores) {
            storeNames.add(store.getStoreName());
        }

        // 리스트뷰 정보 설정
        ListView list = findViewById(R.id.ListView4);
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Store selectedStore = sungsiStores.get(position);
                Intent intent = new Intent(SungsiActivity.this, MapInfoActivity.class);
